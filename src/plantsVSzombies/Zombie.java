...
// 2 shields
	...
	public void work(Bord[] bord, double clock) {
		if (_location != ((int) (_location / 9)) * 9) {
			if (!_isWorking && bord[_location - 1].getPlantType() != 0 && bord[_location - 1].getPlantType() != 4) {
				this.startWorking();
				return;
			} else if (_isWorking
					&& (bord[_location - 1].getPlantType() == 0 || bord[_location - 1].getPlantType() == 4)) {
				this.stopWorking();
			}
		}
		if (_isWorking) {
